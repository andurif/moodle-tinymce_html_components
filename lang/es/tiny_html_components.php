<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * Strings for component 'tiny_html_components', language 'es'.
 *
 * @package tiny_html_components
 * @author 2020 Anthony Durif
 * @copyright 2020 Anthony Durif, Université Clermont Auvergne
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

$string['accordion'] = 'Acordéon';
$string['accordion_nb'] = 'Número de lineas :';
$string['accordion_show'] = 'Dejar lineas ocultas';
$string['addhtml_components'] = 'Agregar componentes HTML';
$string['alert'] = 'Alerta';
$string['alert_close'] = 'Posibilidad de cerrar la alerta :';
$string['alert_danger'] = 'Error/peligro (rojo)';
$string['alert_dark'] = 'Dark (negro)';
$string['alert_info'] = 'Información (cyan)';
$string['alert_primary'] = 'Información principal (azul)';
$string['alert_secondary'] = 'Información secondaria (gris)';
$string['alert_success'] = 'Exito (verde)';
$string['alert_type'] = 'Tipo de alerta :';
$string['alert_warning'] = 'Advertencia (amarillo)';
$string['basic_components'] = 'Componentes básicos';
$string['button'] = 'Botón';
$string['button_dropdown'] = 'Agregar menú al botón';
$string['button_dropdown_nb'] = 'Número de opciones en el menú:';
$string['button_nb'] = 'Número de botones :';
$string['button_size'] = 'Talla de los botones :';
$string['button_size_block'] = 'Bloc';
$string['button_size_lg'] = 'Grande';
$string['button_size_med'] = 'Medio';
$string['button_size_sm'] = 'Pequeño';
$string['button_style'] = 'Estilo de botón :';
$string['button_style_full'] = 'Lleno';
$string['button_style_outline'] = 'Contorno';
$string['button_tooltip'] = 'Agregar un tooltip :';
$string['button_tooltip_bottom'] = 'Abajo';
$string['button_tooltip_left'] = 'Izquierda';
$string['button_tooltip_pos'] = 'Posicion :';
$string['button_tooltip_right'] = 'Derecha';
$string['button_tooltip_top'] = 'Sobre';
$string['button_type'] = 'Tipo de botón :';
$string['card'] = 'Viñeta';
$string['card_background'] = 'Tipo de fondo :';
$string['card_classic'] = 'Classic (blanco)';
$string['card_display'] = 'Visualización de las viñetas :';
$string['card_display_horizontal'] = 'Lado a lado';
$string['card_display_vertical'] = 'En columna';
$string['card_disposition'] = 'Disposición de las viñetas : ';
$string['card_disposition_info'] = 'Número máximo lado a lado: XL (1), L (2), M (3), S (4)';
$string['card_disposition_line'] = ' por linea';
$string['card_image'] = 'Imagen';
$string['card_nb'] = 'Número de viñeta :';
$string['card_orientation'] = 'Orientación de las viñetas : ';
$string['card_orientation_landscape'] = 'Paisaje';
$string['card_orientation_portrait'] = 'Retrato';
$string['choice_legend'] = 'Elija el componente';
$string['component'] = 'Componente :';
$string['component_placeholder'] = 'Elija el componente...';
$string['custom_components'] = 'Componentes personalizados';
$string['custom_components_back'] = 'Volver a la lista de sus componentes personalizados';
$string['custom_components_construct'] = 'Construcción del componente';
$string['custom_components_content'] = 'Contenido (HTML) del componente';
$string['custom_components_content_help'] = 'Contenido en HTML del componente que construirás lanzando el editor. Eso se insertará después de la selección del componente.';
$string['custom_components_create'] = 'Crear un componente persoalizado';
$string['custom_components_delete'] = 'Borrar el componente';
$string['custom_components_delete_confirm'] = '¿Está seguro de que desea eliminar permanentemente este componente?';
$string['custom_components_delete_wrong_user'] = 'No eres el propietario de este componente. Entonces no puedes borrarlo.';
$string['custom_components_edit'] = 'Editar el componente';
$string['custom_components_link'] = 'Administre sus componentes personalizados';
$string['custom_components_name'] = 'Nombre del componente';
$string['custom_components_name_help'] = 'Nombre que desea darle al componente. Este nombre se mostrará en la lista de componentes disponible en el complemento del editor.';
$string['custom_components_preview_msg'] = '<div class="alert alert-info">Vista previa disponible después de la validación !</div>';
$string['custom_components_title'] = 'Componentes HTML personalizados';
$string['html_components:desc'] = 'Agregar componentes HTML';
$string['html_components:title'] = 'Agregar componentes HTML';
$string['insert'] = 'Agregar HTML';
$string['jumbotron'] = 'Jumbrotron';
$string['nav'] = 'Menu de navigacion';
$string['nav_nb'] = 'Número de iperenlaces :';
$string['pluginname'] = 'Componentes HTML para Tiny';
$string['pluginname_help'] = 'Extensión para ayudar usted a agregar componentes (bootstrap por ejemplo) en el editor Tiny.';
$string['preview_desc'] = 'El avance del componente le permite visualizar los componentes que se mostrarán en la página. Esta visualización puede ser un poco diferente al render final según el tema, las opciones, etc...';
$string['preview_legend'] = 'Avance';
$string['privacy:metadata:tiny_html_components_custom'] = 'Una tabla que almacena componentes personalizados que se pueden utilizar en el editor TinyMCE';
$string['privacy:metadata:tiny_html_components_custom:content'] = 'El código HTML que representa el componente';
$string['privacy:metadata:tiny_html_components_custom:name'] = 'El nombre del componente';
$string['privacy:metadata:tiny_html_components_custom:userid'] = 'El ID de usuario del usuario que creó el componente';
$string['suggestion'] = '';
$string['title'] = 'Agregar componentes HTML';
